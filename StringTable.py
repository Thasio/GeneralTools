...
class StringTable():
    ...
    def draw(self):
        ...
        if required_space + self.indent + \
           self.space * (self.columns - 1) <= self.width:
            for row in range(len(self.rows)):
                line = ' ' * self.indent + self.rows[row][0]
                for column in range(1, self.columns):
                    line += ' ' * (self.space + max_value_in_column[column-1]
                                   - len(self.rows[row][column-1])) \
